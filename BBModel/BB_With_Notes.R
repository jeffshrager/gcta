# How to run (in Mac OSX)
# from command line
#   start R (by typing 'R' and hitting enter)
#   source('/path/to/BB_With_Notes.R')
# from RStudio - note you will need to change the working directory
# using setwd('/path/to/desired/directory') if you want the data dumped 
# somewhere specifically. Further note that R does not understand the '~' symbol and you
# must use 'Users' instead, e.g. setwd('/Users/ConnorSweetnam/Documents/CC/gctasim/BBModel')
# Otherwise it will dump it in a folder named 'results' in your current working directory 
# (you can type getwd() to see your current directory)
#   source('/path/to/BB_With_Notes.R')
#   alternatively click the 'source' button in RStudio

# Typically the message 'There were 50 or more warnings (use warnings() to see the first 50)'
# appears at the end of the run. These are safe to ignore.
...
# WW1 = vector to contain result of 1-compa(R[1,,1:2]) for each simulation, the posterior probs
...
# Give you prob. of inequalities where E is a set of parameters of indepdent betas.
  # CS - So in the example above E[1,1] is alpha and E[1,2] is beta to calculate a beta distribution?
  ...
  compa<-function(E){
    ...
    # ?? I guess these are the params of a beta function, but why 2x2 when Beta only needs 2 params ??
    ...
    # ?? What will xx end up representing ?? What's the second row ?? (What's the first row??)??
      # CS - The second row of E is the distribution of successes and failures in the control arm 
      # CS - The first row of E is the distribution of successes and failures in the experimental arm
  ...
  compa2<-function(E){
    ...
    # 0.01 ~ 0 = Pr(Ek-|NEk+) 
    # 0.99 ~ 1 = Pr(NEk-|NEk+) # What if being pos makes it NE, but being neg makes it Ek? like a switch
...
# prediction functions (don't read this! :-)
  ...
  # ?? Where does the 2000 comes from ?? There appear to be 400 subjects. 
  # 2000 -- number of simulated trials.
  
  W1=rep(0,2000) # p values for each sim
  WW1=rep(0,2000) # result of 1-compa(R[1,,1:2]) per sim, the posterior probs
  ...
  MMMM<-array(0,c(2000,400,3)); # 3 dimensional array of biomarkers per patient per sim
        # i.e. MMMM[2,1,] gives the first patient's biomarker signature for the second trial
  ...
  #SS=400 # number of subjects patients

# 3 is number of biomarkers
# 2 is number of outcomes +-
# 4 number of arms
...
# possibly a piece of code that was later decided not to be used
...
# markers, outcome, arm, when the patient was randomized
...
# W1 and WW1 both store the level of evidecnce at the completion of the trial
# W1 is the pv, the other postior 
    ...
    if(stst==1){
      save(list=ls(), file = paste0(getwd(),"/results/eigthA.N.",as.integer(cluster.id),".RData"))
    }
    if(stst==2){
      save(list=ls(), file = paste0(getwd(),"/results/eigthB.N.",as.integer(cluster.id),".RData"))
    }
    if(stst==3){
      save(list=ls(), file = paste0(getwd(),"/results/eigthA1.N.",as.integer(cluster.id),".RData"))
    }
    if(stst==4){
      save(list=ls(), file = paste0(getwd(),"/results/eigthB1.N.",as.integer(cluster.id),".RData"))
    }
    if(stst==5){
      save(list=ls(), file = paste0(getwd(),"/results/eigthC.N.",as.integer(cluster.id),".RData"))
    }
