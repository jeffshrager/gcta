...
def run(settings):
    ...
    for k in range(settings.param("trial_n")):
        ...
        with g.as_default():
            ...
            W_conv1 = generate_tf_variable([6, 10], 'noisy')
            b_conv1 = generate_tf_variable([10], 'noisy')

            intermediate_x = tf.matmul(x, W_conv1) + b_conv1

            W_conv2 = generate_tf_variable([10, 11], 'noisy')
            b_conv2 = generate_tf_variable([11], 'noisy')

            y = tf.matmul(intermediate_x, W_conv2) + b_conv2
