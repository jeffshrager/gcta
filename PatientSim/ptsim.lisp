; (load (compile-file "ptsim.lisp"))

#|

Create simulated patient data to test Asher's model(s)
Based originally on Jeff's ../LispModel/simed.lisp

We use data created by R's survsim package to create the time-to-event
models both for patient arrival (diagnosis) and patient recurrence.

Uses precomputed time-to-event data from saved in a file called
tte.dat We just uniformly randomly draw from that data to get the tte
numbers when we need them ...

To create the tte.dat file, in R (from the manual linked above):
...
Here's a mod of the above for the first distribution of people showing
up with a diagnosis. Here time is years, so we're getting an age distrubution.

### A cohort with 1000 subjects, with a maximum follow-up time of 100
### years and one covariate for age of dx, following a bernoilli
### distribution, and a corresponding beta of -0.4, -0.5, -0.6 and
### -0.7 for each event. 

sim.data <- mult.ev.sim(n=1000, foltime=100, dist.ev=c('lnorm','lnorm'),
anc.ev=c(0.7,0.8),beta0.ev=c(5,7),anc.cens=10, beta0.cens=4.4,
z=list(c("unif",0.2,2.8)), beta=list(c(2,3)), x=list(c("lnorm", 0.9)), nsit=2)

### Plot:
ggplot(subset(sim.data, time < 100),aes(x=time)) +  geom_histogram()
### or:
ggplot(sim.data,aes(x=time)) +  geom_histogram()

### Dump:
write.table(sim.data, "time2dx.xls", append = FALSE, sep = "\t", col.names = TRUE)

### And these are time-to-recurrance under treatments A vs. B:

sim.data <- mult.ev.sim(n=1000, foltime=60, dist.ev=c('lnorm','lnorm'),
anc.ev=c(0.7,0.8),beta0.ev=c(5,7),anc.cens=10, beta0.cens=3.5,
z=list(c("unif",0.2,2.8)), beta=list(c(2,3)), x=list(c("lnorm", 0.9)), nsit=2)
ggplot(sim.data,aes(x=time)) +  geom_histogram()
write.table(sim.data, "time2rec_A.xls", append = FALSE, sep = "\t", col.names = TRUE)

sim.data <- mult.ev.sim(n=1000, foltime=60, dist.ev=c('lnorm','lnorm'),
anc.ev=c(0.7,0.8),beta0.ev=c(5,7),anc.cens=10, beta0.cens=4.0,
z=list(c("unif",0.2,2.8)), beta=list(c(2,3)), x=list(c("lnorm", 0.9)), nsit=2)

# How many actual events are there (as opposed to censors)? (Status==1)
length(sim.data$time[which(sim.data$status==1)])

# Historgram the times of events
hist(sim.data$time[which(sim.data$status==1)])

# Historgram the times of non-events events
hist(sim.data$time[which(sim.data$status==0)])

# Plot all the time data (regardless of status)
ggplot(sim.data,aes(x=time)) +  geom_histogram()

# Simpler Historgram the times of non-events events
hist(sim.data$time)

# Output the data into a tab separated file:
write.table(sim.data, "time2rec_B.xls", append = FALSE, sep = "\t", col.names = TRUE)
...
;;; ======================================
;;; From the lme4 experimental set
