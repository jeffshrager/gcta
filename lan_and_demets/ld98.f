c     Program ld98.f, using subroutines GLAN and LANDEM
c
c     Performs computations related to group sequential boundaries 
c     using the spending functions.  Interactive input routine 
c     has four options which compute:
...
c     and LANDEM computes bounds given a spending function.  The 
c     long input routine is designed to be run interactively, but 
c     can be run non-interactively.  
c     
c     A copy of the output is sent to file called LANDEM.OUT, which
c     is overwritten each time the program is started.
c     
c     References:
...
c      t2 is the second or information time scale, for covariances.
c      t3 is t2 divided by the maximum information, if needed.
...
c     
      program ld98
c
c     Number of interim analyses is limited to 25.
      integer ndim
      parameter(ndim=25)
c     
      double precision t(ndim),t2(ndim),t3(ndim),tmax
      double precision za(ndim),zb(ndim),drift(ndim)
      double precision pstp(ndim),qpos(ndim),qneg(ndim)
      double precision pd(ndim),pe(ndim)
      double precision power(ndim)
      double precision alpha(2),conf,side,pr,qrcum,value
      integer n
c     
      double precision limit(2),est,phi(2)
      integer i,nopt,ndrifs,iuse(2),iinf
      character char
c
c     Copy output to a file.  Status UNKNOWN overwrites 
c     the file if it exists, creates the file if it doesn't.
      open(unit=10,status='UNKNOWN',file='LANDEM.OUT')
c     
c     Print header message.
      write(6,*)
      write(6,*) ' Program for computations related to group ',
     .     ' sequential boundaries using spending functions.'
      write(6,*)
c     
 10   continue
c-----------------------------------------------------------------------
c     Subroutine input returns values for these variables.
      call input(nopt,n,ndim,t,iinf,t2,t3,tmax,alpha,side,iuse,phi,
     .     za,zb,ndrifs,drift,conf,value,pd,pe)
c     
c-----------------------------------------------------------------------
c     Based on value of nopt, call routines for computation.
      if (nopt.eq.1) then
c        Output bounds from spending function.
         call outld(n,alpha,iuse,phi,side,iinf,t,t2,pd,za,zb)
c     
c     Note: if there is a second time scale, use it for computing 
c           probabilities; otherwise, t and t2 are the same.
c           In case t2 isn't scaled to (0,1], user has entered
c           a maximum value tmax and t2 = t3/tmax.
      else if (nopt.eq.2) then
c        Return drift for given bounds and power.
c        Note: the drift return produces exit probability conf at t(n).
c        If t(n) < 1 (or t3(n) < 1) this is not the study's power.
         call adrift(n,t3,za,zb,conf,drift)
         call glan(n,t3,za,zb,drift(1), pstp,qpos,qneg,est,pr)
         call outgl(n,pr,drift(1),est,iinf,t,t2,tmax,za,zb,
     .        pstp,qpos,qneg,qrcum)
c     
      else if (nopt.eq.3) then
c        Probabilities from bounds, possibly with non-zero drift.
         do 50 i=1,ndrifs
            call glan(n,t3,za,zb,drift(i), pstp,qpos,qneg,est,pr)
            call outgl(n,pr,drift(i),est,iinf,t,t2,tmax,za,zb,
     .           pstp,qpos,qneg,qrcum)
            power(i) = qrcum
 50      continue
         if (ndrifs .gt. 1) then
c           Prompt for graphics.
            write(6,*)
            write(6,*) ' Would you like to see a graph? (1=yes,0=no)'
            read(5,100) char
 100        format(10a)
...
c        Confidence limits from bounds and final statistics value.
         call ci(conf,value,n,t3,za,zb, limit)
...
c     Prompt for return to start.
      write(6,*)
      write(6,*) ' Would you like to start again? (1=yes,0=no)'
      read(5,200) char
 200  format(10a)
      if ((char.eq.'1').or.(char.eq.'y')) then
         go to 10
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine input(nopt,n,ndim,t,iinf,t2,t3,tmax,alpha,side,iuse,
     .     phi,za,zb,ndrifs,drift,conf,value,pd,pe)
c
...
c     truncation value for bounds, optional drift parameters, power, 
c     or confidence level. 
c     
      double precision t(*),t2(*),t3(*),za(*),zb(*),drift(*)
      double precision pd(*),pe(*),alpha(*), phi(*)
      double precision side, conf, value, tmax
...
c
      ...
      parameter(maxnn=50)
c     
      double precision tzb(maxnn), tpd(maxnn), tpe(maxnn)
      double precision zninf, ztrun
      integer i,iequl,iside,ispnd,itrun,isymm,ifile,ibad,irex,nold
      character char
c     
c     The 'save' statement causes all local variables to 
c     retain their values between calls.
      save
c     
      data inter/-1/,irex/0/
c     
...
c        If the program is re-executing, the analysis times and/or
c        the boundaries do not have to be re-entered.  Only used for
c        interactive runs!
         write(6,*)
         write(6,*) ' Re-executing.'
         irex = 1
      end if
c
c     Choose whether to compute bounds, drifts, probabilities or 
c     confidence limits:
 100  if (inter.eq.1) then
         write(6,*) ' Enter number for your option:'
         write(6,*) ' (1) Compute bounds using a spending function.'
         write(6,*) ' (2) Compute drift given power and bounds.'
         write(6,*) ' (3) Compute probabilities given bounds and drift.'
         write(6,*) ' (4) Compute confidence interval.'
      end if
      read(5,*) nopt
c     Echo back the chosen option.
      if (nopt.eq.1) then
     ...
     .        ' and drift parameters, if desired.'
      else if (nopt.eq.4) then 
...
c     
c     If re-executing, should the same times be used?
      if (irex.eq.1) then
         nold = n
         write(6,*) ' Same analysis times? (1=yes,0=no)'
         read(5,150) char
 150     format(10a)
         if ((char.eq.'1').or.(char.eq.'y')) then
c           Jump to code for bounds.
            goto 1090
         end if
      end if
c     
...
c     How many interim analyses?
 200  if (inter.eq.1) write(6,*) ' Number of interim analyses?'
      read(5,*) n
      write(6,*) n,' interim analyses.'
c     
c     Number of looks should be < ndim in calling routine.
      if (n.le.0) then
...
c        Compute n equally spaced times from 1/n to 1.
         do 1010 i=1,n
            t(i) = real(i)/real(n)
 1010    continue
c        If t() somehow got set invalid values, we don't want
c        to re-compute equal spacing in an infinite loop.
         iequl = 0
      else
c        Enter one time for each look.
         if (inter.eq.1) write(6,*) ' Enter times of interim ',
     .        'analyses: (>0 & <=1)'
         read(5,*)  (t(i),i=1,n)
      end if
c     
...
c        If interactive, re-input.
         if (inter.eq.1) goto 1000
c        If noninteractive, program stops.
         stop
      end if
c     
c        Second time scale equal to first by default.
         do 1050 i=1,n
            t2(i) = t(i)
 1050    continue
c     
c     If re-executing, should same bounds be used?
 1090 if ((irex.eq.1).and.(nopt.ne.1).and.(n.eq.nold)) then
         write(6,*) ' Same bounds? (1=yes,0=no)'
         read(5,1095) char
 1095    format(10a)
         if ((char.eq.'1').or.(char.eq.'y')) then
c           Jump to code for drift, power, confidence.
            goto 4000
         end if
      end if
c     
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc     
ccccccccccccccccccccc DETERMINE BOUNDS ccccccccccccccccccccccccc
c     Options 2, 3 and 4 decide whether to use spending fct.
...
c     bounds or enter a set of bounds manually.
 ...
 1110    format(10a)
         if ((char.eq.'1').or.(char.eq.'y')) then
            ispnd = 1
            if (irex.eq.1) then 
               write(6,1115) (t(i),i=1,n)
 1115          format(' Spending function will determine bounds', 
     .              ' at times ', 25(f8.3,1x))
            end if
         else if ((char.eq.'0').or.(char.eq.'n')) then
            ispnd = 0
...
c           If noninteractive, assume spending function!
            ispnd = 1
         end if
      end if
c---------------------------------------------------------------
c     Determine bounds using spending function 
      if (ispnd.eq.1) then
c     
c     Second Time Scale
c        Will exact or estimated information be used?  This is 
c        only useful when bounds are being determined from a
c        spending function.
         if (nopt.eq.1) then
            if (inter.eq.1)
     .           write(6,*) ' Do you wish to specify',
     .           ' a second time/information scale? (e.g. the',
     .           ' inverse of parameter variance or',
     .           ' number of events, as in Lan & DeMets 89?) ',
     .           ' (1=yes, 0=no)'
            read(5,1250) char
 1250       format(10a)
            if ((char.eq.'1').or.(char.eq.'y')) then
...
c           Invalid responses force one time scale only!
               write(6,*) ' Invalid response.  No second scale.'
               iinf = 0
            end if
            if ((inter.eq.1).and.(iinf.eq.1)) 
     .           write(6,*) ' First time scale will be used in ',
     .           'the spending function. Second time scale will ',
     .           'estimate covariances.'
c     
c           If information is being used, enter.
 1300       if (iinf.eq.1) then
               if (inter.eq.1) write(6,*) ' Information:'
               read(5,*) (t2(i),i=1,n)
               write(6,1320) (t2(i),i=1,n)
 1320          format(' Information ',25(f7.3,1x))
c     
...
c        Total type I error probability.
 2000    if (inter.eq.1) write(6,*) ' Overall significance level?',
     .        ' (>0 and <=1)'
         read(5,*) alpha(1)
         if ((alpha(1).le.0.0).or.(alpha(1).gt.1.0)) then
            write(6,*) ' Invalid response.'
            write(6,*) ' Alpha must be > 0 and <= 1.'
c           If running interactively, re-prompt.
            if (inter.eq.1) goto 2000
c           If running noninteractively, out of range value
c            sets alpha = 0.05! 
            alpha(1) = 0.05
c           Print warning if alpha was out of range.
            write(6,*) ' Alpha has been set to 0.05.'
         end if
         write(6,2010) alpha(1)
 2010    format('  alpha = ',f5.3)
c     
c        Upper and lower bounds, upper only, or asymmetric?
 2100    if (inter.eq.1) write(6,*) ' One(1), two(2)-sided ',
     .        'symmetric, or asymmetric(3) bounds?'
         read(5,2110) char
 2110    format(10a)
         if ((char.eq.'1').or.(char.eq.'o')) then
            ...
            side  = 2.d0
            isymm = 1
         else if ((char.eq.'3').or.(char.eq.'a')) then
            side  = 2.d0
            isymm = 0
c     
         else
...
c           If running noninteractively, invalid responses
c              are assumed to be two-sided symmetric!
            side  = 2.d0
            isymm = 1
         end if
c     
 ...
 2120       format(1x,f2.0,'-sided test')
            ...
            write(6,2125) side
 2125       format(1x,f2.0,'-sided asymmetric test')
            write(6,*) ' Note: bounds will be approximations only.  ',
     .           ' Nomimal exit probs may be slightly too large.'
         end if
c     
         if (isymm.eq.1) then
c           alpha(2) and iuse(2) are unused, set null values.
            alpha(2) = 0.0
            iuse(2) = 0
         else
c     
c        For asymmetric bounds, Type I error for lower bound.
 2200       if (inter.eq.1) then
               write(6,*) ' Specify LOWER bound first.'
               write(6,*) ' Enter alpha for lower bound:'
            end if
            read(5,*) alpha(2)
c     
c           Check 0 < alpha(2) < alpha(1).
            if ((0.0.ge.alpha(2)).or.(alpha(2).ge.alpha(1))) then  
               write(6,*) ' Lower alpha out of range!'
               if (inter.eq.1) goto 2200
c              Non-interactive splits alpha.
               alpha(2) = alpha(1)/2.d0
            end if
c     
c           Subtract to get upper alpha(1).
            alpha(1) = alpha(1) - alpha(2)
c     
c           Output upper and lower alpha.
            write(6,2220) alpha(2), alpha(1)
 2220       format(' Lower alpha=',f6.3,' Upper alpha=',f6.3)
c     
c           Now set up both use functions.
c           iuse(2) = -1 indicates it hasn't been set.
            iuse(2) = -1
c     
c           Prompt for lower use function.
            if (inter.eq.1) 
     .           write(6,*) ' Enter use function for lower bound:'
         end if
c        
c-----------------------------------------------------------------------
c        Select use function from programmed choices.
 2300    if (inter.eq.1) then
            write(6,*) ' Use function? (1-5)'
            write(6,*) ' (1) OBrien-Fleming type'
            write(6,*) ' (2) Pocock type'
            write(6,*) ' (3) Power family: alpha * t^phi'
            write(6,*) ' (4) Hwang-Shih-DeCani family'
            if ((side.eq.2.d0).and.(isymm.eq.1)) 
     .           write(6,*) ' (5) List other choices (if any)'
         end if
         read(5,*) iuse(1)
c     
c     Other use function choices can be added here if
c     they are also programmed in subroutine alphas().
         if (iuse(1) .gt. 4) then 
c     
c        There are two possible definitions of two-sided OBF bounds.
c        The lower bound can be the reflection of the alpha/2 one-
c        sided upper bound: this means using a slightly different 
c        spending rate for one and two sided bounds.  This is the 
c        default option above.  Alternatively, the spending 
c        rate can be the same, which means the alpha/2 one-sided 
c        bound and upper alpha two-sided bound are different.  There
c        is very little difference in any case.
            if ((side.eq.2.d0).and.(isymm.eq.1)) then
               write(6,*) ' (6) Alternate OBF'
c     
            else
               write(6,*) ' No other valid choices have been',
     .              ' programmed.  Select from 1,2,3,4.'
            end if
            read(5,*) iuse(1)
         end if
c     
c     Enter parameter for spending function family if needed.
         if (iuse(1).eq.3) then
            if (inter.eq.1) 
     .           write(6,*) ' Enter exponent for power family (phi>0)'
            read(5,*) phi(1)
            if (phi(1).le.0.d0) then
               write(6,*) ' Parameter must be >0!'
c              Set non-positive values to 1.0
               phi(1) = 1.d0
            end if
            write(6,2350) phi(1)
c     
         else if (iuse(1).eq.4) then
            if (inter.eq.1) 
     .           write(6,*) ' Enter parameter for Hwang-Shih-DeCani',
     .           ' family (|phi|>0)'
            read(5,*) phi(1)
            if (phi(1).eq.0.d0) then
               write(6,*) ' Parameter must be nonzero!'
c              Set zero values to 1.0
               phi(1) = 1.d0
            end if
            write(6,2350) phi(1)
c     
 2350       format(1x,'Parameter is ', f6.3)
         end if
c     
         write(6,*) ' Use function alpha-star',iuse(1)
c     
c        For asymmetric bounds, jump back for the upper use function.
         if (iuse(2).lt.0) then
            iuse(2) = iuse(1)
            phi(2)  = phi(1)
            if (inter.eq.1) then
               write(6,*)
               write(6,*) ' Enter use function for upper bound:'
            end if
            goto 2300
         end if
c-----------------------------------------------------------------------
...
c        Does the design include a truncation point?
         if (inter.eq.1) write(6,*) ' Do you want to truncate the',
...
c     
c        Prompt for truncation point.
         if (itrun.eq.1) then
            if (inter.eq.1) write(6,*) ' Enter absolute value',
     .           ' of truncation point: '
            read(5,*) ztrun
            ztrun = dabs(ztrun)
            write(6,2450) ztrun
 2450       format(' Bounds will be truncated at ',f5.2)
            ...
            ztrun = -1.d0
            write(6,*) ' Bounds will not be truncated.'
         end if
c     
c================================================================
c     Evaluate bounds by calling landem
         if (isymm .eq. 1) then
            call landem(n,0.d0,alpha(1),phi(1),t,t2,side,iuse(1),ztrun, 
     .           pe,pd,za,zb)
         else
c           Asymmetric bounds.
c           Compute upper asymmetric bounds.
            call landem(n,0.d0,alpha(1),phi(1),t,t2,1.d0,iuse(1),ztrun, 
     .           pe,pd,za,zb)
c           Store bounds and probabilities in temporary variables.
            do 2800 i=1,n
               tzb(i) = zb(i)
               tpe(i) = pe(i)
               tpd(i) = pd(i)
 2800       continue
c           Compute lower asymmetric bounds.
            call landem(n,0.d0,alpha(2),phi(2),t,t2,1.d0,iuse(2),ztrun, 
     .           pe,pd,za,zb)
c           Recall stored values.
            do 2850 i=1,n
               za(i) = -zb(i)
               zb(i) = tzb(i)
               pe(i) = pe(i) + tpe(i)
...
c        
c     Users may input bounds from a file, here 'BNDS.INP', instead
c     of inputting from the screen or standard input.  
c     
c***  open(unit=20,status='UNKNOWN',file='BNDS.INP')
...
c      end if
c        
c        Delete next line if above has been uncommented.
         ifile = 0
         if (ifile.eq.0) then
...
c           This is only reached if program has been modified.
...
c     
c---------------------------------------------------------------
c     Echo back bounds and times
 4000 if (iinf.eq.1) then
         write(6,*) '          Time                     Bounds '
         do 4050 i=1,n
            write(6,4060) t(i),t2(i),za(i),zb(i)
 4050    continue
 4060    format(4x,f5.2,4x,f9.3,8x,f7.4,4x,f7.4)
      else
         write(6,*) '       Time                Bounds '
         do 4070 i=1,n
            write(6,4080) t(i),za(i),zb(i)
 4070    continue
 4080    format(4x,f5.2,12x,f7.4,4x,f7.4)
      end if
c     
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc     
...
c     
      if (nopt.eq.2) then
c        Enter desired power for Option (2).
...
cccccccccccc COMPUTING POWER CURVES cccccccccccccccccccccccccccc
      if (nopt.eq.3) then
c     For Option (3), are drift parameters to be used?
...
c     
 ...
 5720       if (inter.eq.1) write(6,*) ' How many drift parameters',
     .           ' do you wish to enter?'
            read(5,*) ndrifs
c           Number of drifts must be > 0 and <= ndim.
            if (ndrifs.le.0) then
               write(6,*) ' Number of drifts must be positive'
               if (inter.eq.1) goto 5720
c              Program terminates if not interactive.
               stop
            else if (ndrifs.gt.ndim) then
               write(6,5730) ndim
 5730          format(' Number must be <=',i3,' or ndim must ',
     .              'be increased in main program.')
c              If running interactively, re-prompt.
               if (inter.eq.1) goto 5720
c              If running noninteractively, program stops.
...
c     
 ...
 5750       format(' Drift parameters: ',25(f8.3,1x))
            write(6,*) ' Drift is equal to the expectation',
     .           ' of the Z statistic when time=1.'
         end if
      end if
c     
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc     
...
c        Confidence interval replaces last bound with value.
     ...
     .        ' statistic (Z value) at the last analysis:'
         read(5,*) value
c     
         write(6,6770) value
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc     
cccccccc RESCALE SECOND TIME SCALE IF NECESSARY cccccccccccccccc
c     When second time scale is not on (0,1], computation with 
c     non-zero drift parameters are incorrect, since the drift 
c     always scaled to (0,1].  If the trial is complete (t[n]=1) 
c     then t2 can be rescaled as t3 = t2/t2[n].  Otherwise, if
c     the second time scale is to be used for covariances, the 
c     user must enter a maximum value.  
c     
c     drift( t(i) ) = drift * t(i)
c     
c     Start with t3 = t2.  (t2=t if only one time scale is used.)
      do 7000 i=1,n
         t3(i) = t2(i)
 7000 continue
c     
c     If second time scale has been entered.
      if (iinf.eq.1) then
c     
         tmax = 0.d0
         write(6,*)
c     
c        Options using non-zero drifts:
         if ((nopt.eq.2).or.(nopt.eq.4.).or.(idrif.eq.1)) then
c           If t(n)=1, t2(n) is maximum of t2.
            if (t(n) .eq. 1.d0) then
               if (inter.eq.1) write(6,*) ' 2nd time scale will ',
     .              'be used to determine covariances.'
               tmax = t2(n)
               do 7050 i=1,n
                  t3(i) = t2(i)/tmax
 7050          continue
            else
c              Should we try to use 2nd scale?
               if (inter.eq.1) write(6,*) ' Do you wish to use ',
     .              'the 2nd time scale to determine covariance?',
     .              ' (1=yes, 0=no)'
               read(5,7060) char
 7060          format(10a)
               if ((char.eq.'1').or.(char.eq.'y')) then
...
c              Invalid responses force one time scale only!
                  write(6,*) ' Invalid response.  2nd scale ignored.'
                  imax = 0
               end if
c     
c              If yes, prompt for maximum of 2nd time scale.
               if (imax.eq.1) then
 7070             if (inter.eq.1) write(6,*) ' Enter the maximum ',
     .                 'value of the second time scale. '
                  read(5,*) tmax
                  if (tmax .le. 0.d0) then 
                     write(6,*) ' Maximum must be positive!'
c                    Set negative values to their absolute value.
                     tmax = dabs(tmax)
                  end if
                  if (tmax .lt. t2(n)) then
                     write(6,*) ' This value has already been',
     .                    ' exceeded.'
c                    If interactive, retry.
                     if (inter.eq.1) goto 7070
c                    Otherwise set tmax = t2(n).
                     write(6,7080) t2(n)
 7080                format(' Setting maximum to ',f9.3)
                  end if
c                 
                  write(6,7100) tmax
 7100             format(' Maximum of second time scale is ',f9.3)
c                 Rescale t2
                  do 7150 i=1,n
                     t3(i) = t2(i)/tmax
 7150             continue
...
c              If (imax.eq.0) ...
c              Even if the 2nd time scale was on (0,1], if the 
c              maximum information value was not entered, set
c              t3 to t, which causes t2 to be ignored!
                  do 7170 i=1,n
                     t3(i) = t(i)
 7170             continue
               end if   
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine outgl(n,pr,drift,est,iinf,t,t2,tmax,za,zb,
     .     pstp,qpos,qneg,qrcum)
c     
c     Print out results from glan.
c     Note: pr, est and pstp are included but not used.
c     Note: t2 is the information time scale if two have 
c           been specified.  
c     
      double precision pr,drift,est,t(*),t2(*),tmax,za(*),zb(*),
     .     pstp(*),qpos(*),qneg(*)
      integer n, iinf
c     
...
c     
      if ((iinf.eq.0).or.(tmax.le.0.d0)) then
         write(6,20)
...
c     
         ...
         do 50 i=1,n
            ...
            qrcum = qrcum + qplsr
c     
            write(6,60) i,t(i),za(i),zb(i),
     .           qplsr,qrcum
 50      continue
 60      format(i3,4x,f7.2,2x, 2(2x,f7.4), 8x,f7.5,11x,f7.5)
      else
c        Output with information time.
         write(6,110) tmax
 110     format(1x,'Maximum information: ',f9.3)
         write(6,120)
 120     format(/' look',5x,'time',3x,'info',6x,'lower',4x,'upper',
     .        4x,' exit  probability ',3x,'cum exit pr'/)
c     
         qrcum = 0.d0
         do 150 i=1,n
            qplsr = qpos(i)+qneg(i)
            qrcum = qrcum + qplsr
c     
            write(6,160) i,t(i),t2(i),za(i),zb(i),
     .           qplsr,qrcum
 150     continue
 160     format(i3,4x,f7.2,2x,f7.2,2x, 2(2x,f7.4), 8x,f7.5,11x,f7.5)
      end if
c     
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     Echo output to file on unit 100
c     
      write(10,1010) n,drift
 1010 format(/5x,'n =',i3,', drift =',f8.4)
c     
      if ((iinf.eq.0).or.(tmax.le.0.d0)) then
         write(10,1020)
 1020    format(/' look',5x,'time',6x,'lower',4x,'upper',4x,
     .        ' exit  probability ',3x,'cum exit pr'/)
c     
         qrcum = 0.d0
         do 1050 i=1,n
            ...
            qrcum = qrcum + qplsr
c     
            write(10,1060) i,t(i),za(i),zb(i),
     .           qplsr,qrcum
 1050    continue
 1060    format(i3,4x,f7.2,2x, 2(2x,f7.4), 8x,f7.5,11x,f7.5)
      else
c        Output with information time.
         write(10,1110) tmax
 1110    format(1x,'Maximum information: ',f9.3)
         write(10,1120)
 1120    format(/' look',5x,'time',3x,'info',6x,'lower',4x,'upper',
     .        4x,' exit  probability ',3x,'cum exit pr'/)
c     
         qrcum = 0.d0
         do 1150 i=1,n
            ...
            qrcum = qrcum + qplsr
c     
            write(10,1160) i,t(i),t2(i),za(i),zb(i),
     .           qplsr,qrcum
 1150    continue
 1160    format(i3,4x,f7.2,2x,f7.2,2x, 2(2x,f7.4), 8x,f7.5,11x,f7.5)
c     
      end if
c     
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine outld(n,alpha,iuse,phi,side,iinf,t,t2,pd,za,zb)
c     
c     landem output for bounds computed from spending function.
c     
      double precision alpha(*),side,t(*),t2(*),pd(*),za(*),zb(*)
...
c     Print out results.
      if (side .eq. 1.d0) then
         write(6,10)
         write(6,20) n,alpha(1),iuse(1)
 10      format(//' This program generates one-sided boundaries.')
 20      format(' n = ',i2/' alpha =',f6.3/' use function = ',i1)
c     
         if (iuse(1) .eq. 1) then
            write(6,3010) alpha(1)
         else if (iuse(1) .eq. 2) then
            write(6,3020) alpha(1)
         else if (iuse(1) .eq. 3) then
            write(6,3030) alpha(1),phi(1)
         else if (iuse(1) .eq. 4) then
            write(6,3040) alpha(1),-phi(1),-phi(1)
         end if
c     
      else if (alpha(2) .le. 0.d0) then
         write(6,30)
         write(6,40) n,alpha(1),iuse(1),iuse(1)
...
c     
         if (iuse(1) .eq. 1) then
            ...
            write(6,3040) alpha(1),-phi(1),-phi(1)
         else if (iuse(1) .eq. 6) then
            write(6,3060) alpha(1)
         end if
c     
      else 
         write(6,50)
         write(6,60) n,alpha(2),alpha(1),iuse(2),iuse(1)
 50      format(//' This program generates two-sided ',
     .        'asymmetric boundaries.')
 60      format(' n = ',i2/' lower alpha =',
     .        f6.3/' upper alpha =',f6.3
     .        /' use function for the lower boundary = ',i1
     .        /' use function for the upper boundary = ',i1)
c     
         if (iuse(1) .eq. 1) then
...
c     
         if (iuse(2) .eq. 1) then
            write(6,3010) alpha(2)
         else if (iuse(2) .eq. 2) then
            write(6,3020) alpha(2)
         else if (iuse(2) .eq. 3) then
            write(6,3030) alpha(2),phi(2)
         else if (iuse(2) .eq. 4) then
            write(6,3040) alpha(2),-phi(2),-phi(2)
         end if
c     
      end if
c
c     Improve labelling?
     ...
     .        '        alpha(i)-alpha(i-1)   cum alpha'
         ...
         do 100 i=1,n
            pr = pr + pd(i)
            write(6,150) t(i),za(i),zb(i),pd(i),pr
 100     continue
...
c     
     ...
     .        '     alpha(i)-alpha(i-1)  cum alpha'
         ...
         do 200 i=1,n
            pr = pr + pd(i)
            write(6,250) t(i),t2(i),za(i),zb(i),pd(i),pr
 200     continue
...
c     Prompt for graphics.
 ...
 500     format(10a)
         if ((char.eq.'1').or.(char.eq.'y')) then
            call graphb(n,t,za,zb,side)
         end if
      end if
c     
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     Print out results to file on unit 100.
      if (side .eq. 1.d0) then
         write(10,1010)
         write(10,1020) n,alpha(1),iuse(1)
 1010    format(//' This program generates one-sided boundaries.')
 1020    format(' n = ',i2/' alpha =',f6.3/' use function = ',i1)
c     
         if (iuse(1) .eq. 1) then
            write(10,3010) alpha(1)
         else if (iuse(1) .eq. 2) then
            write(10,3020) alpha(1)
         else if (iuse(1) .eq. 3) then
            write(10,3030) alpha(1),phi(1)
         else if (iuse(1) .eq. 4) then
            write(10,3040) alpha(1),-phi(1),-phi(1)
         end if
c     
      else if (alpha(2) .le. 0.d0) then
         write(10,1030)
         write(10,1040) n,alpha(1),iuse(1),iuse(1)
 1030    format(//' This program generates two-sided ',
     .        'symmetric boundaries.')
 1040    format(' n = ',i2/' alpha =',f6.3
     .        /' use function for the lower boundary = ',i1
     .        /' use function for the upper boundary = ',i1)
c     
         if (iuse(1) .eq. 1) then
            write(10,3015) alpha(1)
         else if (iuse(1) .eq. 2) then
            write(10,3020) alpha(1)
         else if (iuse(1) .eq. 3) then
            write(10,3030) alpha(1),phi(1)
            ...
            write(10,3060) alpha(1)
         end if
c     
      else 
         write(10,1050)
         write(10,1060) n,alpha(2),alpha(1),iuse(2),iuse(1)
 1050    format(//' This program generates two-sided ',
     .        'asymmetric boundaries.')
 1060    format(' n = ',i2/' lower alpha =',
     .        f6.3/' upper alpha =',f6.3
     .        /' use function for the lower boundary = ',i1
     .        /' use function for the upper boundary = ',i1)
c     
...
c     
         if (iuse(2) .eq. 1) then
            write(10,3010) alpha(2)
         else if (iuse(2) .eq. 2) then
            write(10,3020) alpha(2)
         else if (iuse(2) .eq. 3) then
            write(10,3030) alpha(2),phi(2)
         else if (iuse(2) .eq. 4) then
            write(10,3040) alpha(2),-phi(2),-phi(2)
         end if
      end if
c     
c     Improve labelling?
      if (iinf.eq.0) then
         write(10,*) '       Time                Bounds ',
     .        '        alpha(i)-alpha(i-1)   cum alpha'
         pr = 0.d0
         do 1100 i=1,n
            pr = pr + pd(i)
            write(10,1150) t(i),za(i),zb(i),pd(i),pr
 1100    continue
 1150    format(4x,f5.2,12x,f7.4,4x,f7.4,9x,f7.5,9x,f7.5)
c     
      else
         write(10,*) '    Time    Information     Bounds ',
     .        '     alpha(i)-alpha(i-1)  cum alpha'
         pr = 0.d0
         do 1200 i=1,n
            pr = pr + pd(i)
            write(10,1250) t(i),t2(i),za(i),zb(i),pd(i),pr
 1200    continue
 1250    format(2x,f5.2,5x,f7.2,4x,f7.4,2x,f7.4,7x,f7.5,7x,f7.5)
      end if
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     
 3010 format(1x,'OBrien-Fleming type:',
     .     ' 2-2*Pnorm(Z_(',f6.3,'/2)/sqrt(t))')
c     
 3015 format(1x,'OBrien-Fleming type:',
     .     ' 4-4*Pnorm(Z_(',f6.3,'/4)/sqrt(t))')
c     
 3020 format(1x,'Pocock type:',f6.3,'*log(1+(e-1)*t)')
c     
 3030 format(1x,'Power family:',f6.3,'*t^',f6.3)
c     
 3040 format(1x,'Hwang-Shih-DeCani family:',
     .     f6.3,'*(1-exp(',f6.3,'*t)/(1-exp(',f6.3,')')
c     
 3060 format(1x,'Alternative OBrien-Fleming type:',
     .     ' 2-2*Pnorm(Z_(',f6.3,'/2)/sqrt(t))')
c     
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     
...
c     Echo output to file on unit 100.
c     
      write(10,1100) 100*conf,limit(1),limit(2)
 1100 format(//2x,f3.0,' percent confidence interval:',
     .     ' (',f7.4,',',f7.4,')')
c     
      write(10,*) ' Drift is equal to the expectation',
     .     ' of the Z statistic when time=1.'
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     
      return
...
c     
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
cccc R E T U R N   D R I F T   F O R   G I V E N   P O W E R  cccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine adrift(n,t,za,zb,conf, drift)
c     Find drift resulting in power "conf".
c     The drift will be that value producing exit probability "conf
c     at time t(n) given the bounds.  If t(n) < 1, this is not the 
c     power of the study.

      double precision t(*),za(*),zb(*),conf,drift(*)
      integer n
      double precision znorm
c     
c     Starting value for drift: there may be better choices.
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine ci(conf,value,n,t,za,zb, limit)
c     
c     Computes confidence intervals, see Kim & DeMets 87.
...
c     value is the Z statistic at the most recent analysis.
...
c     
      double precision conf,value,t(*),za(*),zb(*),limit(*)
      integer n
c     
      double precision znorm
      double precision zcrit,target,tzbn
      integer i
c     
c     Save zb(n).
      tzbn  = zb(n)
...
c     tests of hypotheses.  Method is a modification of Armitage,
...
c     lnn dimensions local vectors in glan. NOTE: if this is changed,
...
c     formula, the inner integral is a normal cdf and the outer
...
c       NOTE: drift MAY NOT be changed without adding code!
c     alpha is the total type I error.
c     t is the information times.
c     t2 is the 'second' time scale, used for the covariance of
c       sequential statistics, usually number of events or subjects.
...
c     
      double precision za(*),zb(*),alpha,phi,t(*),t2(*),pe(*),pd(*),
...
c     Calculate probabilities according to use function.
      call alphas(iuse,nn,alpha,phi,side,t,pe,pd)
c     
...
c     bound from the previous analysis.
         ...
         zb(i) = yb(i)/sdproc(i)
c     
c     If a truncation point is being used, check to see if it
c     applies and recompute probabilities if necessary.
         if (zb(i) .gt. ztrun) then
            zb(i) = ztrun
            pd(i) = side * qpos(zb(i)*sdproc(i),last,nints(i-1),ya(i-1),
     .           yb(i-1), h, stdv(i))   
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     
      subroutine alphas(iuse,nn,alpha,phi,side,t,pe,pd)
c
...
c     znorm is the inverse standard normal cdf.
c     
      double precision alpha,side,phi,t(*),pe(*),pd(*)
      integer iuse,nn
c     
      integer i
...
c     
c     Code for two families of spending functions: the power family
c     and the Hwang-Shih-de Cani family.
         else if (iuse .eq. 3) then
            pe(i)=(alpha/side)*(t(i) ** phi)
c     
         else if (iuse .eq. 4) then
            pe(i)=(alpha/side)*
     .           (1.d0 - dexp(-phi*t(i)))/(1.d0 - dexp(-phi))
c     
c        iuse=5 doesn't indicate a spending function.
c     
         else if (iuse .eq. 6) then
c     
c           When iuse=6, a two-sided OBF using the same spending
c           function as the one-sided OBF is computed.
            pe(i)=
     .       (1.d0-pnorm(znorm(1.d0-(alpha     )/2.d0)/dsqrt(t(i))))
c     
c     
c     Add other spending function options here: e.g.
c        else if (iuse.eq.7) then . . .
         else
...
c     
 90   format('Large change in bounds, possible error.',i4)
...
c     
      ...
      integer i,xloc,yloc,yloc1,yloc2,flag
      integer hspace,vspace
      parameter(hspace=42,vspace=25)
      character*1 window(0:hspace,0:vspace),xaxis(0:hspace),symbol
      parameter(symbol='*')
...
c     
c     Title for x-axis.
c     write(6,*) '                     Information Fraction'
      write(6,*) '                             Time        '
      write(6,*)
c     
