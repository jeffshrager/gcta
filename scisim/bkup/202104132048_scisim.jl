...
# otherwise, we treat all the variants they have. The treatment either
# has to have no attached trials, or an attached trial must be in
# phase23. The reason we might get here even though there are trials
# for this vn, is that the available trials might be full up.
...
function freetreat(pt)
    for vn in findall(v->v==1,pt.variant_vector)
        ...
        txs != [] ? treatwith(pt,txs[rand(1:end)]) : log!(@sprintf("No available treatments for pt %s, var %s!\n",pt.id,vn))
    end
end
...
function simrun(;overrides=[],loglevel=0,contextdict=Dict()) # Watch as variants accrue and health declines
    ...
    let nyears = @hyp(:nyears), treatment_threshold = @hyp(:treatment_threshold), nvars = @hyp(:nvars)
        ...
        if loglevel == 0
            ...
            plt.savefig(@sprintf("scisim_%s.png",Dates.format(now(),"mmddyyyy_HHMM_SSssss")))
            println("\n\nTrial results:\n")
...
function run(overkey::Symbol,from::Real,to::Real,step::Real;result_key=:mean_survival,n=10,otheroverrides=[],color="blue",label="none")
...
function elog(label,contextdict)
    open("experiment_history.json", "a") do io
...
function test(;test=:trials,n=10,otheroverrides=[])
    if test==:trials
        run(:tx_effect_size_mean_for_positive_variant,0.0,0.8,0.05,n=50,otheroverrides=[[:trialsp,true],[:npts,500]],color="red")
        run(:tx_effect_size_mean_for_positive_variant,0.0,0.8,0.05,n=50,otheroverrides=[[:trialsp,false],[:npts,500]],color="blue")
    elseif test==:finetrials
        run(:tx_effect_size_mean_for_positive_variant,0.0,0.5,0.025,n=n,otheroverrides=[[:trialsp,true]],color="red")
        run(:tx_effect_size_mean_for_positive_variant,0.0,0.5,0.025,n=n,otheroverrides=[[:trialsp,false]],color="blue")
    elseif test==:phase23min
        run(:phase23_min_pts,10,100,10,n=n,color="blue",otheroverrides=otheroverrides,label="phase23_min_pts")
    elseif test==:txs
        run(:treatment_threshold,0.0,1.0,0.1,n=n,otheroverrides=[[:trialsp,false]],color="red",label="No Trials")
        run(:treatment_threshold,0.0,1.0,0.1,n=n,otheroverrides=[[:trialsp,true]],color="blue",label="Trials")
    elseif test==:newtxs
        run(:tx_invention_death_fraction,0.0,1.0,0.1,n=n)
    elseif test==:txs_per_var
        run(:txs_per_variant_limit,1,10,1)
    elseif test==:fast # Just for testing
        run(:txs_per_variant_limit,1,10,1,n=10,otheroverrides=[[:npts,10]])
    end
end
