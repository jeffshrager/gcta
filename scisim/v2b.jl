...
# v2b includes contra-recommendations.
...
function rvtb(algorithm_certainty,distance_function,correction_distance_function)
    ...
    return(pro_distances,con_distances)
end
    
# This is used for the pros and the cons, just with different params
...
function gather_opinions(label,alphafn,betafn,ecert,acert,n_experts,distance_function,correction_distance_function,n_recs)
    ...
    trace!(@sprintf("--- %s ---\n",label))
    # Experts first:
    weights=[pdf(Beta(alphafn(ecert),betafn(ecert)),i/g_n_txs) for i in 1:g_n_txs]
    if label=="con"; weights=reverse(weights); end
    trace!(@sprintf("eweights=%s\n",weights))
    erecs=([sample(g_txs,Weights(weights),n_recs;replace=false) for x in 1:n_experts]) # expert recommendations
    ...
    # Algorithm next:
    weights=[pdf(Beta(alphafn(acert),betafn(acert)),i/g_n_txs) for i in 1:g_n_txs]
    if label=="con"; weights=reverse(weights); end
    trace!(@sprintf("aweights=%s\n",weights))
    arec=sample(g_txs,Weights(weights),n_recs;replace=false)
