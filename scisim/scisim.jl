# You're in the middle of making phase 1 always succeed, and saving the
# phase 1 pts as synthetic controls for phase23 of the same drug.

# The main driver is simrun(), but we have meta-drivers test() and run()
# Put calls at the tail of this file.

# BUGS (!!!BBB)
#    Trials are using a bad estimate of survival for the success or failure of the trial!
#       Need to create MATCHED PATIENTS!

# ToDo:
#    Leo: Dynamic contrarian: Change to the WORST drug for you (incl. off-variant targets) (make sure can never pass others)
#    Allow simrun to take npts etc.
#    Dynamic GCTA change drugs when better are discovered 
#    Use assymptotic health fns
...
# occurrance probability (vop, per cycle==cycle). Each variant also has
# an effect size (ves), which actually the mean and sd of the gaussian
# effect size, again, per cycle.  So, as patients develop variants over
...
mean_survival(pts)=mean(map(pt->((last(pt.history)!=0.0) ? @hyp(:ncycles) : (findall(h->h==0.0,pt.history)[1])),pts))
mean_health(pts)=mean(map(pt->pt.health,pts))
...
tx_target(tx) = tx.effect_model[1][1] # FFF Need to generalize for cocktails!

# Give the indeces of the status=non-false teatments for a given variant.
live_treatment_indexes_for_variant(vn) = findall(tx->tx.status!=false&&vn==tx_target(tx),gtxs)
...
# trial. In the present model, phase 1 trials always succeed -- they
# are jsut used to get synthetic contol data, and to power for the
# phase 23. Once in phase23, anyone who can't get into the trial can
# use the drug (on expanded access). Patients who are enrolled in
# either a phase 1 or phase 2 trial only get the trial treatment,
# whereas patients who are NOT in a trial can get any drug that is in
# phase 2 or beyond. So an advantage of going into a phase 1 trials is
# that you get drug before anyone else. The disadvantage is that
# you're stuck with that one drug. One question is, why would anyone
# who could get the drug anyway, bother going into a phase 2/3 trial?
# The answer is that they wouldn't, but that's not the world we live
# in, that's the world we're comparing to! There is another factor,
# which is that any patient who CAN go into a trial MUST go into a
# trial (under the :strict policy). In :strict there is a health
# cutoff for the trial, which is based on the phase 1 analysis.  Any
# patient with the target mutation who passes the health cutoff MUST
# (under :strict) go into the 23 trial.  The :strict policy is
...
# When a patient is added to a trial, a dictionary gets created whose
# keys are the patient, and whose values is a trial_patient_record
# (tpr), which records the performance of patients on the trial. This
# is used to create the synthetic controls for phase23 trials (from
# Phase1 records), and to show trial history.

mutable struct tpr; pt; started_at; health_at_start; ended_at; health_at_end; end
mutable struct ct; id; txs; pts; phase; ptsneeded; duration_per_patient; tprs; end

global all_trials = []
global phase23_trials = []
global completed_trials = []
global failed_trials = []
global gthis_cycle = 0 # Used for tracing deep inside
...
function trial_result(trial)
    let matched_pts = matched_pts(trial)
        # Is this a fair comparison? We're comparing pts on the trial
        # with the health of matched pts NOW.  If they are dead, well,
        # so be it, otherwise, they've been being treated for a long
        # time, and may be pretty good by now. Maybe we have to
        # compare a similar period of treatment, otherwise, either the
        # drugs will always succeed because every else is dead (mean
        # health = 0) or every else is fine (mean health = 1)
        (trial.pts == [] || matched_pts == []) ? :uncomputable : mean_health(trial.pts)/mean_health(matched_pts) 
    end
end

# Matched patients are a random sample of the same number of pts as in
# the trial, and who have the target variant, but have never had the
# drug.

function matched_pts(trial)
    @printf("\n\nMatching for trial: %s\n",trial.id)
    let target_vn = tx_target(trial.txs[1]),
        pts_with_variant = pts_with_variant(target_vn,trial.pts),
        available_matched_pts = setdiff(pts_with_variant,trial.pts)
        # End of let list (Why can't people just use Lisp?!)
        @printf("There are %s trial pts, %s pats with the variant, and %s matched patients.\n", length(trial.pts), length(pts_with_variant), length(available_matched_pts))
        # Needs to pick the same number, without replacement, if possible, or all, if less
        return(length(available_matched_pts)>=length(trial.pts) ? rand(available_matched_pts,length(trial.pts)) : available_matched_pts)
    end
end
...
function new_tx(vn)  # FFF Rationalize the effect size, and add a
    ...
    if @hyp(:trialsp)
        nct = ct(1+length(all_trials),[ntx],[],1,@hyp(:phase1_ptsneeded),@hyp(:phase1_duration_per_patient), Dict())
        ntx.status=nct
        push!(all_trials,nct)
    end
...
function treat(pt)
    ...
    if ct == false
        ...
        if ct.duration_per_patient == pt.trial_cycles
            ...
            pt.trial = false
            ct.tprs[pt].ended_at = gthis_cycle
            ct.tprs[pt].health_at_end = pt.health
            ...
            if (length(ct.pts)==ct.ptsneeded)
                # If the trial failed, shut down the treatment (only phase23 trials can fail)
                # (FFF UUU This logic should cull p1 trials first, since they can't fail)
                let result = trial_result(ct)
                    if  (ct.phase == 23) && (result == :uncomputable) || (result < 1.0) # ??? uncomputeable trials are assumed to have failed
...
function find_trial_or_freetreat(pt)
    ...
    if goodtrials != []
        ...
        ct.tprs[pt]=tpr(pt, gthis_cycle, pt.health, nothing, nothing)
...
function simrun(;overrides=[],tracelevel=0,contextdict=Dict()) # Watch as variants accrue and health declines
    ...
    let ncycles = @hyp(:ncycles), treatment_threshold = @hyp(:treatment_threshold), nvars = @hyp(:nvars)
        for cycle in 1:ncycles
            global gthis_cycle = cycle # This is used in various places deep inside to trace history
            trace!(@sprintf("\n-------- %s --------\n",cycle))
            for pt in gpts
        ...
        # Reporting:
        if tracelevel == 0
            plt.plot([[[pt.history for pt in gpts][i][j] for i in 1:ghyps[:npts][1]] for j in 1:ncycles]) # UUU Array rotation
            plt.ylabel("Health")
            plt.xlabel("Cycles from Diagnosis")
...
simrun(contextdict=Dict(:x_label=>"Test"), overrides=[[:trialsp,true],[:txs_per_variant_limit,5],[:npts,250]])
