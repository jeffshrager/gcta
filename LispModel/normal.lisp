...
;;;; 20151015: Jeff Shrager added scaling functions
...
;;; Treat normal-random-integer as a Z and scale for mean and dev.

(defun scaled-normal (&key n m d)
  (loop for n below n
	collect (+ m
		   (* d
		      (/ (- (normal-random-integer 10000) 5000) 5000.0)))))

(defun test-sn (&optional (m 0.5) (d 0.2))
  (loop with a = (make-array 100)
	as v in (scaled-normal :n 10000 :m m :d d)
	if (> v 0)
	do (incf (aref a (truncate (* v 100))))
