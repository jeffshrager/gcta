...
(defstruct (patient (:print-function (lambda (p s k) (format s "[PATIENT:~a]" (patient-id p)))))
  id
  birth-year
  gender
...
(defstruct (drug (:print-function (lambda (d s k) (format s "[Drug:~a]" (drug-name d)))))
  ...
  (with-open-file
   ...
   (loop for j = (handler-case (json:decode-json-from-source i) (end-of-file nil))
     ...
     (setf *hypotheses-matrix* (list-to-2d-array (rest (assoc :BEST-MATRIX j)))
           *drug-dict* (rest (assoc :DRUG-DICT j))))))

;; returns the most recent (i.e. highest number) of the list of subfolders in the "RL/experiments/" directory
(defun find-most-recent-run (&aux (maximum 0))
  (setf experiment-names (directory "../experiments/*.*"))
  (loop for path in experiment-names
    when (and (parse-integer (namestring (first (last (pathname-directory path)))) :junk-allowed t) (> (parse-integer (namestring (first (last (pathname-directory path)))) :junk-allowed t) maximum))
          do (setf maximum (parse-integer (namestring (first (last (pathname-directory path)))) :junk-allowed t)))
  (return-from find-most-recent-run maximum))
...
(defparameter *experiment-number* (find-most-recent-run))
(defparameter *classifier-output* 
  (format nil "../experiments/~a/~a/~a" *experiment-number* *run-label* (param-reader "../parameters.json" :classifier-output)))
...
(defparameter *scoring-function* (make-keyword (rest (assoc :scoring-function *parameters*))))
...
(defun update-k-np (px)
  ;; This version gives a positive bump when you cover all the
  ;; mutations in a tumor, and a negative bump when you don't. The
  ;; adversities are ignored for the moment.
  (if (set-difference (patient-tumor px) (reduce #'union (loop for tx in (patient-drugs px) collect (drug-markers tx)))) 
      (decf (patient-k px) 5) ;; we've missed at least one tumor mutation
    (incf (patient-k px) 5) ;; we fully target all the tumor mutations
    )
  ;; Now chop the ceiling and floor
  (setf (patient-k px) (max (min (patient-k px) 100) 0))
  ;; Finally, stochastically mutate the tumor
...
(eval-when 
 ...
 (defmacro redrug (px initexpr &body new-drugs-body)
   ...
   `(let* ((ks-5 (histscan-fast-5 :scan ,px #'cdar)) ;; the reverse chron of k scores
      ...
      (if (= 1 histlen) 
	  (setf drugs ,initexpr) ;; Not enough info yet, just do the initialization function.
	;; Record history, and change when the patient is declining
	(progn
	  ...
	  (if (< (first ks-5) (second ks-5))
	      (setf drugs ,@new-drugs-body)
	    "This is the null condition -- drugs will remain the same in this case")
	  ) ;; Close the non-initialization condition
	) ;; Close (if (= 1 histlen) ...  
      ;; Emergency check in case the user f'ed up and didn't provide any drugs at all!
      (unless (and drugs (every #'(lambda (drug) (member drug *drugs*)) drugs))
	(error "Something's wrong in REDRUG: ~a returned ~s, which is either nil, or all of them aren't drugs!" 
	       *txrule* drugs))
      ;; DRUGS should now be either changed by the body, or not. If it
      ;; turns out to be the same as what's already there, then we
      ;; record a :NO-CHANGE
...
(defun assess-and-treat (px)
  ;;(format t "~%========we have successfully called assess and treat with TXRULE : ~a ========~%" *txrule*)
  ...
  (case *txrule*
	(:test (redrug px '(:foo) '(:foo)))
	(:random-binary-cocktail 
	 (redrug px (n-random 2 *drugs*) 
		 (n-random 2 *drugs*)))
	(:random-monotherapy
	 (redrug px (n-random 1 *drugs*) 
		 (list (nth (random (length *drugs*)) *drugs*))))
	(:untried-monotherapy
	...
	(:untried-binary-cocktail
	 (redrug px (n-random 2 *drugs*) 
		 (untried-binary-cocktail drugs previous-drugs cds)))
	(:targeted-therapy ;; Currently starting on binary cocktail
	 (redrug px (n-random 2 *drugs*) 
		 (choose-drugs (patient-tumor px)))) 
	(:hypotheses-sampled
	 (redrug px (hypotheses-sampled (patient-tumor px))
		 (hypotheses-sampled (patient-tumor px))))
	(:hypotheses-top-2
	 (redrug px (hypotheses-top-2 (patient-tumor px))
		 (hypotheses-top-2 (patient-tumor px))))
	(:sanity-check
	 (redrug px (sanity-check (patient-tumor px))
		 (sanity-check (patient-tumor px))))
	(t (error "Bad txrule in assess-and-treat ~a" *txrule*))
	)
  ;; Stop if they're dead
  ;;(when (zerop (patient-k px)) (throw 'done px))
  (when 
    (or (= 100 (patient-k px)) (zerop (patient-k px)))
    (throw 'done px)
    )
  )
...
(defun hypotheses-sampled (tmuts &aux ds-index)
  ;;(format t "~%========we have successfully called hyp-sampled========~%")
  (if *hypotheses-matrix*
      (progn ;;(format t "we obviously have a weight matrix ~%")
	     ;; Setup weight matrix:
	     ...
	     (loop for mut in tmuts do (setf (aref pmuts 0 (position mut *markers*)) 1))
	     ...
	     (loop with r = nil
		   ...
		   until (= 2 (length r)) ;; Early stopping rule if we find 
...
(defun hypotheses-top-2 (tmuts &aux ds-index)
  ;;(format t "~%========we have successfully called hyp-top-2========~%")
  (if *hypotheses-matrix*
      (progn ;;(format t "we obviously have a weight matrix ~%")
	     ;; Setup weight matrix:
	     (setf *mut-mat* (make-array '(6 6) :initial-element 0))
	     ...
	     ;; This is tricky if it can't pick 2 bcs they're all zeros except one (or none)!
	     ;; In that case we either choose randomly (if there are none) or duplicate (for 1).
	     (mapcar #'name->drug (mapcar #'cdr (palist-choose-top-2 (mapcar #'cons drug-p-vector *drug-dict*)))))
    ;; No hyps: Just throw the dice!
    (n-random 2 *drugs*)
    )
  ) 
...
(defun palist-choose (palist)
  (let* ((ns (mapcar #'car palist))
     (min (apply #'min ns))
     (absmin (abs min))
     (newns (mapcar #'(lambda (n) (+ n absmin)) ns))
     (norm (float (reduce #'+ newns)))
     (target (/ (random 1000) 1000.0)))
    (loop for (nil . label) in palist
      as p in newns
      with sum = 0
      do (incf sum (/ p norm))
      (if (< target sum)
...
(defun palist-choose-top-2 (palist &aux (maxn -1000))
  (first-n 2 (sort palist #'> :key #'car)))
...
(defun one-px (ncycles)
  ;;(format *mrhlog* "---   --- --- --- --- --- --- ---~%")
  (catch 'done ;; "I see dead people. And alive people. We hope."
    (loop with px = (genpx)
	  as *date* below ncycles
	  do 
	  ;;(print (list 'a (patient-drugs px)))
	  (assess-and-treat px)
	  ;;(print (list 'b (patient-drugs px)))
	  (case *scoring-function* 
		(:parametric (update-k px))
		(:non-parametric (update-k-np px)))
	  finally (return px))))
...
(defun run-trial ()
  ...
  (setf *pxs* nil)
  (with-open-file
   (o (format nil "../experiments/~a/~a/pxs.json" *experiment-number* *run-label*) :direction :output :if-exists :append :if-does-not-exist :create)
   (setq *foo*
...
(defun report (&aux (ts+ (incf *fcounter*)))
  ...
  (clrhash *tmr->drugs*)
  (with-open-file 
   (o (format nil "../experiments/~a/~a/~a-p-drugs-per-tumor.xls" *experiment-number* *run-label* (get-universal-time))
  ...
  (let* ((kss (mapcar #'(lambda (px) (histscan-fast-5 :scan px #'cdar)) *pxs*))
    ...
    (with-open-file 
     (o (format nil "../experiments/~a/~a/~a-trajectories.xls" *experiment-number* *run-label* ts+) :direction :output :if-exists :append :if-does-not-exist :create)
  ...
  #+nil ;; !!!!!!!!!!! Temporarily out !!!!!!!!!!!!!
  (with-open-file 
   (o (format nil "../experiments/~a/~a/~a-drug-targets.xls" *experiment-number* *run-label* ts+) :direction :output :if-exists :append :if-does-not-exist :create)
   (print *param-header* o) (terpri o)
   (loop for drug in *drugs*
...
(defun run ()
  ...
  (if (eq *txrule* :sanity-check)
      (if *hypotheses-matrix*
      (progn 
      (setq *txrule* :hypotheses-sampled)
      (format t "~% ************* OVER-RULING :SANITY-CHECK WITH :hypotheses-sampled *************** ~%"))))
  (setf *drugs* 
    (case (print *txrule*)
  ...
  (with-open-file
   (*mrhlog* (format nil "../experiments/~a/~a/~a" *experiment-number* *run-label* *sim-output*) 
         :direction :output :if-exists :append :if-does-not-exist :create)
   ;;(format *mrhlog* "Patient  TimeStep    TumorCode   ComPos  Drug    DrugCode    KNow    KPrev   KDiff~%")
   (setf *mrhlog-as-list* nil)
   (with-open-file 
    (*log* (format nil "../experiments/~a/~a/stats.xls" *experiment-number* *run-label*) 
       :direction :output :if-exists :append :if-does-not-exist :create)
  ...
  ;; Dump genetics
  (with-open-file 
   (o (format nil "../experiments/~a/~a/genetics.tsv" *experiment-number* *run-label*) :direction :output :if-exists :append :if-does-not-exist :create)
...
(defun main () 
  ...
  (untrace)
  ;;(trace hypotheses-top-2 assess-and-treat)
